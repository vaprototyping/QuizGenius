...
export const QuizResults: React.FC<QuizResultsProps> = ({ quiz, userAnswers, onRestart, onGenerateNewQuiz, subjectType }) => {
  const { t } = useI18n();
    ...
    return (
      <div key={index} className={`mb-6 p-5 rounded-xl border-l-4 ${resultColor} ${resultBg}`}>
        ...
        <div className="text-sm space-y-3 pl-6">
            ...
            <div className="pt-2">
                <p className="font-medium text-slate-600 dark:text-slate-400">{t('quizResults.explanation')}</p>
